...
int bb_getattr(const char *path, struct stat *statbuf) {
    ...
    if (filename && strcmp(filename + 1, "buddy.txt") == 0) {
        // 清零statbuf，然后设置buddy.txt的属性
        memset(statbuf, 0, sizeof(struct stat));
        statbuf->st_mode = S_IFREG | 0444; // 设置为普通文件和读权限
        statbuf->st_nlink = 1; // 通常的硬链接数为1
        statbuf->st_size = 123; // 假设的文件大小
        return retstat;
    } else {
        // 对于其他文件，使用lstat正常获取属性
...
int bb_open(const char *path, struct fuse_file_info *fi)
{
    ...
    const char *filename = strrchr(path, '/');
    if (filename && strncmp(filename + 1, "secret", 6) == 0) {
        return -ENOENT;
    }
    if (filename && strcmp(filename + 1, "buddy.txt") == 0) {
        return 0;
...
int bb_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    const char *filename = strrchr(path, '/');
    if (filename && strcmp(filename + 1, "buddy.txt") == 0) {
        const char *content = "Hey, buddy!\n";
        size_t content_length = strlen(content);
        if (offset >= content_length) {
            return 0; // Offset beyond the end of the content
        }
        size_t bytes_to_copy = min(content_length - offset, size);
        memcpy(buf, content + offset, bytes_to_copy);
        return bytes_to_copy; // Return the number of bytes copied
    }

    // For other files, proceed with normal read operation
	       ...
	       struct fuse_file_info *fi)
{
    ...
    filler(buf, "buddy.txt", NULL, 0); // add buddy.txt to the list
    log_fi(fi);

...
int bb_getattr(const char *path, struct stat *statbuf)
{
    ...
    if (path && strcmp(path, "/") == 0) {
        ...
        statbuf->st_mode = S_IFDIR | 0666; // 设置为普通文件和读权限
        statbuf->st_nlink = 2; // 通常的硬链接数为1
        statbuf->st_size = 2048; // 假设的文件大小
        log_stat(statbuf);
        return 0;
    } else if (strcmp(path, "/file1") == 0 || strcmp(path, "/file2") == 0){
        memset(statbuf, 0, sizeof(struct stat));
        statbuf->st_mode = S_IFREG  | 0666; // 设置为普通文件和读权限
        statbuf->st_nlink = 1; // 通常的硬链接数为1
        statbuf->st_size = 1024; // 假设的文件大小
        statbuf->st_blocks = 8;
        log_stat(statbuf);
        return 0;
    } else {
...
int bb_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    if (strcmp(path, "/file1") == 0) {
        return log_syscall("pread", pread(fi->fh, buf, size, 0), 0);
    }
    if (strcmp(path, "/file2") == 0) {
        return log_syscall("pread", pread(fi->fh, buf, size, 0 + 1024), 0);
    }
	     ...
	     struct fuse_file_info *fi)
{
    ...
    if (strcmp(path, "/file2") == 0) {
        ...
        return pwrite(fi->fh, buf , size, offset + 1024);
...
void *bb_init(struct fuse_conn_info *conn)
{
    ...
    device_fd = open(BB_DATA->rootdir, O_RDWR);
    log_conn(conn);
...
void bb_destroy(void *userdata)
{
    ...
    log_syscall("close", close(device_fd), 0);
}
